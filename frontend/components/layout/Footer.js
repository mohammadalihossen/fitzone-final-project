      ...
      <header className="sticky top-0 z-50 bg-dark border-b border-border">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center gap-6 h-16">
            ...
            {/* ✅ Mobile spacer: search bar hidden থাকলে এটা flex-1 নিয়ে icons-কে right-এ রাখে */}
                  ...
                  <div className="absolute right-0 top-10 w-48 bg-dark-2 border border-border shadow-2xl z-[60] rounded-lg overflow-hidden animate-in fade-in slide-in-from-top-2">
                    {isAuthenticated ? (
                      <>
                        ...
                        <Link
                          href="/dashboard"
                          className="flex items-center gap-2 px-4 py-3 text-white/80 hover:text-primary hover:bg-dark-3 text-sm transition-all"
                        ...
                        <button
                          ...
                          className="w-full flex items-center gap-2 px-4 py-3 text-red-400 hover:bg-red-500/10 text-sm text-left transition-all"
                        >
                      ...
                      <Link
                        href="/auth/login"
                        className="flex items-center gap-3 px-4 py-4 text-white hover:bg-dark-3 text-sm font-bold transition-all"
                        onClick={() => setUserMenuOpen(false)}
                      >
          ...
          <div className="hidden md:flex items-center gap-8 py-2.5 border-t border-border/50">
            {navLinks.map(link => (
              <Link
                key={link.href}
                href={link.href}
                className={`text-sm transition-all duration-300 hover:text-primary hover:translate-x-0.5 ${pathname === link.href ? 'text-primary font-black' : 'text-muted font-medium'}`}
              >
                {link.label}
              </Link>
            ))}
        ...
        <div className={`md:hidden fixed inset-x-0 top-[64px] bg-dark-2/95 backdrop-blur-xl border-b border-border shadow-2xl transition-all duration-300 ${menuOpen ? 'opacity-100 translate-y-0 visible' : 'opacity-0 -translate-y-4 invisible'}`}>
          <div className="p-6 flex flex-col gap-6">
            ...
            <form onSubmit={handleSearch}>
              <div className="relative">
                ...
                <input
                  ...
                  value={searchQuery}
                  onChange={e => setSearchQuery(e.target.value)}
                  className="w-full bg-dark-3 border border-border text-white placeholder-muted pl-10 pr-4 py-2.5 text-sm rounded-xl focus:outline-none focus:border-primary transition-colors"
                />
              </div>
            </form>
            ...
            <nav className="flex flex-col gap-1">
              ...
              {navLinks.map(link => (
                <Link
                  ...
                  className={`px-4 py-3 rounded-xl text-sm font-bold transition-all active:scale-95 ${pathname === link.href ? 'bg-primary text-black' : 'text-white hover:bg-dark-3 hover:text-primary'}`}
                >
                  {link.label}
                </Link>
              ))}
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                onClick={() => { setActiveModal('about'); setMenuOpen(false); }}
                className="flex items-center justify-center p-3 bg-dark-3 border border-border rounded-xl text-white font-bold text-xs uppercase tracking-widest hover:border-primary transition-all"
              >
                About
              </button>
              <button
                onClick={() => { setActiveModal('contact'); setMenuOpen(false); }}
                className="flex items-center justify-center p-3 bg-dark-3 border border-border rounded-xl text-white font-bold text-xs uppercase tracking-widest hover:border-primary transition-all"
              >
                Support
              </button>
